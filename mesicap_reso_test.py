...
from mesicap_reso import XORDataset
...
def create_moe_kernels():
        ...
        uint idx [[thread_position_in_grid]])
    {
        ...
        for (uint d = 0; d < (uint)emb_dim; d++) {
            expert_inputs[expert_offset + uint(pos) * uint(emb_dim) + d] = input[input_offset + d];
        }
...
class OptimizedMoEFeedForward(nn.Module):
    def __init__(self, cfg):
        ...
        self.register_buffer('expert_token_indices',
            torch.zeros(self.num_experts, self.max_tokens_per_expert, dtype=torch.int32))
    ...
    def forward(self, x):
        ...
        with torch.mps.profiler.profile():
            ...
            # Simple, correct recombine on-device
            ...
            # Debug prints: first few counts and sum
            total_assigned = sum(int(c) for c in counts_cpu)
            if getattr(self, '_debug_print_once', True):
                print(f"[MoE-Metal] counts[:min(8,E)]={counts_cpu[:min(8, len(counts_cpu))]} sum={total_assigned} capacity={self.max_tokens_per_expert}")
                self._debug_print_once = False

            for expert_id, n_tok in enumerate(counts_cpu):
                if n_tok <= 0:
                    continue
                # Slices remain on device
                ...
                out_flat.index_add_(0, token_idx, expert_out * weights.unsqueeze(-1))
...
def train(
    ...
    encoder.load("tiny_encoder.json")
