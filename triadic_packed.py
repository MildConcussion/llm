...
class PackedTriadicShardDataset(Dataset):
    """
    Memory-mapped shard of Triadic-encoded tokens with strict doc-boundary windowing.
    ...
    def __init__(
        ...
        # Determine dtype from meta if available
        self._tokens_dtype = 'uint16'
        meta_path = self.shard_dir / 'meta.json'
        if meta_path.exists():
            try:
                with open(meta_path, 'r') as f:
                    meta = json.load(f)
                self._tokens_dtype = meta.get('dtype', 'uint16')
            except Exception:
                self._tokens_dtype = 'uint16'
    ...
    def _ensure_open(self):
        ...
        if self.tokens is None:
            self.tokens = np.memmap(self._tokens_path, dtype=self._tokens_dtype, mode='r')
...
class MixedPackedTriadicDataset(Dataset):
    """
    ...
    def __init__(
        self,
        datasets: List[PackedTriadicShardDataset],
...
def build_mixed_dataset(
    ...
    for r in dataset_roots:
        ...
        if not shard_dirs:
            continue
        shard_datasets = [
            PackedTriadicShardDataset(sd, seq_length=seq_length, stride=seq_length // 2, pad_id=pad_id)
    ...
    return MixedPackedTriadicDataset(per_root_datasets, policy=policy, weights=weights)
...
def _estimate_windows_for_lengths(lengths: np.ndarray, seq_length: int, stride: int) -> int:
    ...
    Mirrors PackedTriadicShardDataset logic but counts only.
    """
...
def build_capped_mixed_dataset(
    ...
    for shard_dirs in selected_per_root:
        if not shard_dirs:
            continue
        shard_datasets = [
            PackedTriadicShardDataset(sd, seq_length=seq_length, stride=stride, pad_id=pad_id)
            for sd in shard_dirs
        ]
...
def _doclevel_split_single_shard(shard_path: Path, train_dir: Path, val_dir: Path, val_ratio: float, rng: np.random.RandomState) -> None:
    ...
    if meta_path.exists():
        try:
            with open(meta_path, 'r') as f:
                meta = json.load(f)
            tokens_dtype = meta.get('dtype', 'uint16')
            has_mask = bool(meta.get('has_loss_mask', has_mask))
        except Exception:
            pass

    tokens = np.memmap(shard_path / 'tokens.mmap', dtype=tokens_dtype, mode='r')
    ...
    for doc_id, L in enumerate(lengths.tolist()):
        ...
        seg = tokens[start:end].astype(tokens.dtype, copy=False)
    ...
    flat_train = np.concatenate(train_tokens).astype(tokens.dtype, copy=False) if train_tokens else np.array([], dtype=tokens.dtype)
    mm = np.memmap(train_sub / 'tokens.mmap', dtype=tokens.dtype, mode='w+', shape=(flat_train.shape[0],))
    mm[:] = flat_train[:]
    mm.flush()
    del mm
    ...
    meta = {
        ...
        'format': 'triadic_docs' + ("_qwen" if has_mask else ""),
    ...
    flat_val = np.concatenate(val_tokens).astype(tokens.dtype, copy=False) if val_tokens else np.array([], dtype=tokens.dtype)
    mm = np.memmap(val_sub / 'tokens.mmap', dtype=tokens.dtype, mode='w+', shape=(flat_val.shape[0],))
    mm[:] = flat_val[:]
    mm.flush()
    del mm
