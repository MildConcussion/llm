...
class XORDataset(Dataset):
    ...
    def __init__(self, data_path: Union[str, Path], seq_length: int = 512,
                 stride: Optional[int] = None, encoder: TriadicEncoder = None):
...
class AsymGQATransformerBlock(nn.Module):
    ...
    def __init__(self, d_model, n_heads, d_ff, rope,
        ...
        self.xor_weight = nn.Parameter(torch.ones(n_heads) * 0.1)
        self.bit_weight = nn.Parameter(torch.ones(n_heads) * 0.1)
    ...
    def forward(self, x, mask=None, key_padding_mask=None, positions: torch.Tensor | None = None, bit_patterns=None):
        B, L, D = x.shape
        ...
        if bit_patterns is not None:
            # bit_patterns: [B, L, 8]
            bit_q = bit_patterns.unsqueeze(1).expand(-1, self.n_heads, -1, -1)
            bit_k = bit_patterns.unsqueeze(1).expand(-1, self.n_heads, -1, -1)

            # XOR distance for bit similarity
            xor_dist = (bit_q.unsqueeze(-2) - bit_k.unsqueeze(-3)).abs().sum(-1)
            xor_resonance = torch.exp(-xor_dist * self.xor_weight.view(1, -1, 1, 1))

            scores = scores + xor_resonance * self.bit_weight.view(1, -1, 1, 1)
...
class MesicapLM(nn.Module):
    ...
    def __init__(self, d_model=512, n_heads=8, n_layers=6,
        ...
        # Update bit lookup table for larger vocabulary
        vocab_size = self.encoder.vocab_size

        # Triadic embedding layers (NEW)
        self.bit_embed = nn.Embedding(256, d_model // 3, dtype=dtype)  # Gray codes only
        self.bpe_embed = nn.Embedding(vocab_size, d_model // 3, dtype=dtype)  # Full vocabulary
        self.sem_embed = nn.Embedding(vocab_size, d_model // 3, dtype=dtype)  # Semantic

        # Bit pattern extraction for XOR operations
        self.bit_extract = nn.Linear(d_model, 8, dtype=dtype)

        # Remove old bit_lut and bit_proj, replace with new projection
        self.embed_proj = nn.Linear(d_model - (d_model % 3), d_model, dtype=dtype)
    ...
    @torch.jit.export
    def get_triadic_embeddings(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """Get triadic embeddings with XOR fusion."""
        B, L = x.shape

        # Extract bit-level features (0-255 range only for actual bytes)
        bit_tokens = torch.clamp(x, 0, 255)
        bit_feats = self.bit_embed(bit_tokens)  # [B, L, d_model//3]

        # BPE features (full vocabulary)
        bpe_feats = self.bpe_embed(x)  # [B, L, d_model//3]

        # Semantic features
        sem_feats = self.sem_embed(x)  # [B, L, d_model//3]

        # XOR fusion: bit ⊕ bpe ⊗ semantic
        xor_mix = torch.tanh(bit_feats + bpe_feats)  # Simulated XOR
        and_gate = torch.sigmoid(sem_feats)  # AND gate

        # Concatenate all three components
        embed = torch.cat([bit_feats, xor_mix * and_gate, sem_feats], dim=-1)

        # Project to model dimension if needed
        if embed.size(-1) != self.d_model:
            embed = self.embed_proj(embed)

        # Extract bit patterns for attention
        bit_patterns = torch.sigmoid(self.bit_extract(embed))

        return embed, bit_patterns
    ...
    def forward(self, x: torch.Tensor, positions: torch.Tensor | None = None,
        ...
        if (x == self.encoder.PAD).all():
            empty_logits = torch.zeros(B, L, self.encoder.vocab_size, device=x.device, dtype=self.param_dtype)
            return empty_logits if not return_cache else (empty_logits, [])

        # Get triadic embeddings instead of bit projection
        h, bit_patterns = self.get_triadic_embeddings(x)
        ...
        if not use_cache:
            ...
            for _ in range(max_len):
                ...
                if debug:
                    ...
                    special = 'END' if token_id == self.encoder.END else ('START' if token_id == self.encoder.START else ('PAD' if token_id == self.encoder.PAD else ''))
    ...
    def _sample_next_token(self, logits, sampler, sampling, top_p, alpha, debug):
        ...
        logits[..., self.encoder.END] = -float('inf')  # Note: END instead of EOS
        logits[..., self.encoder.PAD] = -float('inf')
        logits[..., self.encoder.NUM] = -float('inf')  # New special token
        logits[..., self.encoder.SEP] = -float('inf')  # New special token
        ...
        if not torch.isfinite(logits).any():
            ...
            logits[..., self.encoder.START] = -float('inf')
            logits[..., self.encoder.PAD] = -float('inf')
            logits[..., self.encoder.NUM] = -float('inf')
            logits[..., self.encoder.SEP] = -float('inf')
...
def train(
    ...
    encoder = TriadicEncoder(vocab_size=2048)
    encoder.load("triadic_encoder.json")

    # Create model once and reuse across stages
    model = MesicapLM(d_model, n_heads, n_layers, seq_length, rope_base, encoder=encoder)
